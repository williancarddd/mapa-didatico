...
export default function ModalMarker() {
    ...
    const [month, setMonth] = useState(1); // Valor inicial para o mês
    const [year, setYear] = useState(2021); // Valor inicial para o ano
    const [loading, setLoading] = useState(false);
    const [gifURL, setGifURL] = useState<string>('');

    const getGifRegressionMonthYear = async () => {
        try {
            setLoading(true);
            const response = await apiMapaDidaticoBackend.get(`/linear-gif/${stationSelected?.id}/${year}/${month}`, {
                responseType: 'arraybuffer'
            });

            if (response.status === 200) {
                const gifBytes = new Uint8Array(response.data);
    ...
    return (
        <>
            <div className="h-screen p-8 mb-4">
                ...
                <section className="flex flex-col justify-center  m-4 pb-4 items-center w-5/5">
                    <div className="border shadow bg-astronaut-950 border-gray-700 p-2 rounded-md">
                        {!gifURL ? '' : <Image src={gifURL} alt={""} width={750} height={600} ref={gifRef}/>}
                        <div className="flex p-5">
                            <button onClick={() => {
                                setMonth((prevMonth) => {
                                    // Incrementa o mês, mas volta para 1 se for dezembro (12)
                                    return prevMonth === 12 ? 1 : prevMonth + 1;
                                })
                            }} type="button" className="text-white bg-blue-700 hover:bg-astronaut-800focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm py-2.5 text-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 inline-flex items-center justify-center w-[300px]">
                            ...
                            <button onClick={() => restartGif()} type="button" className="text-white bg-blue-700 hover:bg-astronaut-800focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm py-2.5 text-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 inline-flex items-center justify-center w-40">
                                De novo
                            </button>
                            <SelectComponent
                                id="yearFilterGif"
